...
`touch_lookback_days` 期間内のイベントは統一されますが、期間外の匿名訪問は処理されないため、長期間匿名利用した後にログインすると古い匿名IDのレコードが残ります。ただし、本モデルはログイン済みユーザーの動向を長期間追跡することが主目的のため、低頻度の full-refresh で対応すれば十分です。
...
- 分析者が気軽に長期間のユーザー行動を追跡できるようになった
    - 長期間にわたる分析の需要を促進し、モデルを更に充実させる好循環を目指す
